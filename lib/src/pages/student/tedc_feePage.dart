...
class TEDCFeePage extends StatefulWidget {
  const TEDCFeePage({Key? key}) : super(key: key);

  @override
  _TEDCFeePageState createState() => _TEDCFeePageState();
}

class _TEDCFeePageState extends State<TEDCFeePage> {
  late FPNUser _user;
  ...
  void initState() {
    ...
    context.read<DataBloc>().add(FetchPaymentsEvent(
        paymentType: PaymentType.TEDC, userId: AuthBloc.uid!));
    super.initState();
  }
  ...
  void showPaymentOptionDialog(BuildContext context) {
    showBarModalBottomSheet(
      ...
      builder: (context) => PaymentOptionsWidget(
          paymentType: PaymentType.TEDC, user: _user),
    );
  }
}
