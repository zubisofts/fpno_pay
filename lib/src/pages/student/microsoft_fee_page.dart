...
class MicrosoftFeePage extends StatefulWidget {
  const MicrosoftFeePage({Key? key}) : super(key: key);

  @override
  _MicrosoftFeePageState createState() => _MicrosoftFeePageState();
}

class _MicrosoftFeePageState extends State<MicrosoftFeePage> {
  late FPNUser _user;
  ...
  void initState() {
    ...
    context.read<DataBloc>().add(FetchPaymentsEvent(
        paymentType: PaymentType.Microsoft, userId: AuthBloc.uid!));
    super.initState();
  }
  ...
  void showPaymentOptionDialog(BuildContext context) {
    showBarModalBottomSheet(
      ...
      expand: false,
      builder: (context) =>
          PaymentOptionsWidget(paymentType: PaymentType.Microsoft, user: _user),
    );
  }
}
